...
// pkgDir is the directory of this package's source, captured at build time.
// The request logger's own log records carry a source location pointing here;
// we drop it so the logged source points at the caller instead. Deriving it
// from our own source file keeps the match correct whether the SDK is built
// from a module cache (whose path carries an @version suffix) or a local
// replace/checkout.
...
func (s *Schema) ReplaceAttr(groups []string, a slog.Attr) slog.Attr {
	...
	case slog.SourceKey:
		...
		if s.SourceFile == "" {
			// Ignore the RequestLogger middleware's own source location so the
			// logged source points at the caller, not this package.
			if pkgDir != "" && strings.HasPrefix(source.File, pkgDir) {
				return slog.Attr{}
			}
			return a
		}
